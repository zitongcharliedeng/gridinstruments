...
generated from literate sources to the store.
...
- **Scripts** (lines 1096--1111) — ES module entry point
  (`/main.ts`), inline GitHub star-count fetch, and an SVG `<defs>`
...
`mountGridOverlay` component from `components/mount-grid-overlay.tsx`.
...
point loads `main.ts`; a small inline script fetches the live GitHub
